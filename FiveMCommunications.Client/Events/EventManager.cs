namespace FiveMCommunications.Client.Events
{
    using System;
    ...
    public class EventManager
    {
		...
		public EventManager()
		{
			this._logger = new FiveMCommunicationsLogger("Events");
		}
		...
		internal async Task<TReturn> Request<TReturn>(string @event, params object[] args)
		{
			...
			try
			{
				...
				{
					this._logger.Trace($"Request Reply: \"{@event}\" with {args.Length} payload(s): {string.Join(", ", args.Select(a => a?.ToString() ?? "NULL"))}");
